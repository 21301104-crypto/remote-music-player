...
class PlaybackService {
  ...
  notify() {
    if (this.notifyCallback) this.notifyCallback();
  }
  ...
  async init() {
    ...
    if (dbCount === diskFiles.length && diskFiles.length > 0) {
      ...
      this.rebuildQueue();
      this.notify();
      console.log(`⚡ [Playback Engine] ${this.masterLibrary.length} canciones cargadas desde SQLite en 2ms.`);
      return;
    }

    console.log(`⏳ [Playback Engine] Sincronizando ${diskFiles.length} archivos...`);
    const enrichedList = [];
